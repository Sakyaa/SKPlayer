...
@implementation SKPlayerStateView

+ (instancetype)shareInstance {
    static SKPlayerStateView *instance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[SKPlayerStateView alloc] init];
    });
    return instance;
}
- (instancetype)init {
    if (self = [super init]) {
        ...
        self.backImage.image        = SKPlayerImage(@"ZFPlayer_brightness");
...
- (void)setIsStatusBarHidden:(BOOL)isStatusBarHidden {
    ...
    [[UIWindow sk_currentViewController] setNeedsStatusBarAppearanceUpdate];
}
